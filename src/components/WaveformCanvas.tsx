...
function gauss(x: number, center: number, width: number, amp: number): number {
  return amp * Math.exp(-Math.pow((x - center) / width, 2));
}
...
export function WaveformCanvas({ param, color, speed, isRunning, value }: WaveformCanvasProps) {
  ...
  const stateRef = useRef({ x: 0, lastX: 0, lastY: 0, timePassed: 0, rng: 0 });
  ...
  useEffect(() => {
    ...
    const draw = (ts: number) => {
      ...
      if (isRunning) {
        stateRef.current.timePassed += delta;
        stateRef.current.rng += 0.07;
        const tMs = stateRef.current.timePassed;
        const pxPerSec = speed * 2;
        const step = (pxPerSec * delta) / 1000;
        let newX = stateRef.current.x + step;
        let wrap = false;
        ...
        if (param === 'ECG') {
          ...
          const amp = 1 + Math.sin(tMs / 3000 + stateRef.current.rng) * 0.06;
          ...
          const brMs = 60000 / rate;
          const phase = (tMs % brMs) / brMs;
          if (phase < 0.04) yOff = 0;
          else if (phase < 0.18) yOff = -28 * smoothstep(0.04, 0.18, phase);
          else if (phase < 0.65) yOff = -28 - 2 * ((phase - 0.18) / 0.47);
          else if (phase < 0.75) yOff = (-30 + 2 * ((phase - 0.18) / 0.47)) * (1 - smoothstep(0.65, 0.75, phase));
          else yOff = 0;
        ...
        if (!wrap && stateRef.current.lastY !== centerY) {
          ctx.beginPath();
        ...
        stateRef.current.lastX = newX;
        stateRef.current.lastY = newY;
      }
